...
# Load .env before anything else so Google credentials are available
try:
...
from sheets_db import SheetsDatabase as Database
...
def cmd_search(args):
    ...
    sheet_url = job_info.get("spreadsheet_url", "") if job_info else ""
...
def cmd_place(args):
    _setup_logging(args.verbose)
    db = Database()
    job_id = hashlib.sha256(args.place_id.encode()).hexdigest()[:16]
    ...
    # Create a job so the Sheets backend provisions a spreadsheet
    db.create_job(job_id, f"place:{args.place_id}")
...
def cmd_resume(args):
    ...
    if not args.quiet:
        _clear_line()
    elapsed = time.time() - start
    sheet_url = job.get("spreadsheet_url", "")
    print(f"\nDone in {elapsed:.1f}s | Places: {stats['places_saved']}/{stats['places_found']} | Reviews: {stats['reviews_saved']:,} | Errors: {stats['errors']}")
    if sheet_url:
        print(f"Google Sheet: {sheet_url}")
...
def cmd_stats(args):
    _setup_logging(args.verbose)
    db = Database()
    print("Fetching job list from Google Drive...")
    ...
    if jobs:
        ...
        for job in jobs:
            ...
            if job.get("spreadsheet_url"):
                print(f"      Sheet: {job['spreadsheet_url']}")
...
def cmd_jobs(args):
    ...
    print("Recent jobs in Google Drive folder:")
    print("-" * 80)
    for job in jobs:
        status_icon = "\u2713" if job["status"] == "done" else "\u25cb" if job["status"] == "running" else "\u2717"
...
def main():
    ...
    st = subs.add_parser("stats", help="Show job statistics from Google Drive")
